...
Part 1 covers an introduction to non-parametric tests and when they might be useful to consider. The lecture recording is available [here](https://dtu-panopto.lancs.ac.uk/Panopto/Pages/Viewer.aspx?id=fb98a72c-5f74-4fc7-bc51-ae3d01751e29) and the slides are available [here](https://modules.lancaster.ac.uk/mod/folder/view.php?id=1785786).

**Part 2: Testing the assumption of normality with two independent groups** 

Part 2 covers how to test the assumption of normality with two independent groups. The lecture recording is available [here](https://dtu-panopto.lancs.ac.uk/Panopto/Pages/Viewer.aspx?id=31ce4852-0c15-42b2-ab69-ae3d01800ef4) and the slides are available [here](https://modules.lancaster.ac.uk/mod/folder/view.php?id=1785786).

**Part 3: The Wilcoxon-rank sum test** 

Part 3 introduces you to the Wilcoxon rank-sum test. This includes when to use this test, the theory behind it, how the test statistic would be calculated manually, how to run the test in R, and how to interpret the output. The lecture recording is available [here](https://dtu-panopto.lancs.ac.uk/Panopto/Pages/Viewer.aspx?id=0affadb4-8523-4841-b467-ae3d0182426a) and the slides are available [here](https://modules.lancaster.ac.uk/mod/folder/view.php?id=1785786).

**Part 4: The Wilcoxon signed-rank test** 

Part 4 covers how to test the assumption of normality with two repeated measures and also introduces you to the Wilcoxon signed-rank test, including when to use the test, the theory behind it, how to calculate the test statistic manually, how to run the test in R and how to interpret the output. The lecture recording is available [here](https://dtu-panopto.lancs.ac.uk/Panopto/Pages/Viewer.aspx?id=3b79c12a-7482-44dc-b513-ae3d017a7bc7) and the slides are available [here](https://modules.lancaster.ac.uk/mod/folder/view.php?id=1785786).

## Pre-lab work

Before the lab, please watch the following [video](https://dtu-panopto.lancs.ac.uk/Panopto/Pages/Viewer.aspx?id=0d934880-0288-4be1-a797-ae3c00f4ecf7). This walks you through how to perform a Wilcoxon rank-sum test and Wilcoxon signed-rank test in R. The R markdown script covered in this video and the required data files can be downloaded [here](https://modules.lancaster.ac.uk/mod/folder/view.php?id=1791571).

## Lab

In the lab, we'll practice running a Wilcoxon rank-sum and Wilcoxon signed-rank test in R. Please download the following zip file which contains two datasets: [apple_study_data.csv](files/Week_16/apple_study_data.csv) and [banana_study_data.csv](files/Week_16/banana_study_data.csv).
...
Here is a [model script](files/Week_16/Week_16_Lab_Model_Script.Rmd)  that produces the answers to the above research questions. I use the word 'model' loosely, as you may have used different functions you've learned over the last two years (and that's absolutely fine!). So don't worry if you haven't used the exact same functions as me. You should end up with the same results and interpretation at the end though.

### Feedback on scripts

Feedback on R scripts submitted by students is available [here](https://modules.lancaster.ac.uk/mod/folder/view.php?id=1785808).
...
###### Group 1:
```{r echo = FALSE} 
worksheet_activity1 <- read.csv("files/Week_16/l6_worksheet1.csv")
...
```{r echo = FALSE} 
worksheet_activity2 <- read.csv("files/Week_16/l6_worksheet2.csv")
